  ...
  function groupMessages (messages: ChatMessage[]) {
    ...
    for (const { message, user, type, date } of messages) {
      ...
      if (grouped.length && last.user.id === user.id) {
        last._messages.push(message)
      } else {
        grouped.push({ user, _messages: [message], type, date })
      }
    }
    return grouped
  }
...
{#each groupMessages($messages) as { type, user, date, _messages }, i (i)}
